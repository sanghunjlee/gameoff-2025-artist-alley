...
func _ready() -> void:
    MerchManager.merch_started.connect(func (_merch_stack): update_ui())
    MerchManager.merch_completed.connect(func (_merch_stack): update_ui())
...
func update_ui() -> void:
    ...
    if MerchManager.current_order != null:
        ...
        row.order_id = 0
        row.order = MerchManager.current_order
        row.is_current = true
        progress_container.add_child(row)

    for order_id in range(1, MerchManager.order_queue.size() + 1):
        var order = MerchManager.order_queue[order_id - 1]
        var row = progress_row_scene.instantiate()
